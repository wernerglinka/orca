...
const sharedMethods = {
  methods: {
    softScroll: function (event, target) {
      ...
      store.toggleVisibility(false); // soft scroll to the target
...
const toTopComponent = {
  ...
  template: `<a v-show="toTopIsVisible" @click="softScroll(event, '#onTop')" id="toTopButton" href="#onTop"><i class="icon icon-arrow-up"></i></a>`,
  mounted: function () {
    'use strict';
...
const navComponent = {
  ...
  methods: {
    toggleMenu: function () {
      ...
      const thisComponent = this; // toggle the menu
      // toggle nav state

      store.toggleVisibility(!this.sharedState.navIsVisible); // add navActive class to the body element
